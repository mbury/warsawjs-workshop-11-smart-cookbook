...
it('dispatches the exactly same action', () => {
  ...
  const action = { type: 'FOO', payload: { data: 1 } };
...
it('dispatches entities action along with the normalized action', () => {
  const store = mockStore({});
  const action = {
    type: 'ENTITY_DETAIL_SUCCESS',
    payload: { data: { id: 2, foo: 'bar' } },
  };
  expect(store.dispatch(action)).toEqual({ ...action, payload: { data: 2 } });
  expect(store.getActions()).toEqual([
    {
      type: 'ENTITIES_RECEIVE',
      entities: { entity: { 2: { id: 2, foo: 'bar' } } },
    },
